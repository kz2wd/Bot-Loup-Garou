...
async def send_player_vote_menu(channel, game_players: list[Player], name: str, placeholder: str, vote_options: list[Player], get_valid_voter_player, timeout: int, on_time_over: callable):
    vote_system = VoteCollector(vote_options, [player for player in game_players if get_valid_voter_player(player) is not None])
    view = TimedSelectView(timeout=timeout, on_time_over=lambda: on_time_over(vote_system))
